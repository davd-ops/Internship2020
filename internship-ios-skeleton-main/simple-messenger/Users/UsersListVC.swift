...
public class UsersListVC: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let detailViewController = UsersDetailVC()
        
        //let selectedRow = users[indexPath.row]
        
        if users.count > indexPath.row {
            detailViewController.title = "\(users[indexPath.row].name) \(users[indexPath.row].surname)"
            detailViewController.userrID = users[indexPath.row].id
            detailViewController.nameLabel.text = "\(users[indexPath.row].name) \(users[indexPath.row].surname)"
            detailViewController.emailLabel.text = users[indexPath.row].email
        }
